...
class cartesian_controller
{
    ...
    ros::NodeHandle n;
    ros::Subscriber desired_twist_sub, observed_twist_sub;
...
cartesian_controller::cartesian_controller()
{
    ...
    desired_twist_sub = n.subscribe("/motor_controller/twist", 1000, &cartesian_controller::callback_desired_twist, this);

    // initialize a subscriber for taking desired twist (here published manually)
    observed_twist_sub = n.subscribe("/kobuki/encoders", 1000, &cartesian_controller::callback_observed_twist, this);

    // initialize a ROS Publisher of type ras_lab1_msgs::PWM
    pwm_pub = n.advertise<ras_lab1_msgs::PWM>("/kobuki/pwm", 1000);
    ...
    Basically, spinOnce() forces all subscriber callbacks, and then 
    returns the control sequence. on the other hand, ros spin keeps 
    the control sequence and keeps forcing callbacks. ros spin is 
    to ensure the program doesnt go out of scope till subscribers are 
    running
    */
}

void cartesian_controller::callback_desired_twist(const geometry_msgs::TwistConstPtr& twist_msg)
{
...
void cartesian_controller::callback_observed_twist(const ras_lab1_msgs::EncodersConstPtr& encoder_msg)
{
